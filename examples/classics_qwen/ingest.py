"""Ingest classics into a Qwen3-VL optimized index with smaller chunking.

Usage:
    python examples/classics_qwen/ingest.py [--writer-url http://localhost:8002]
"""
...
SOURCES = [
    ...
    },
    {
        ...
        "doc_prefix": "zhuangzi",
        "chunking": "paragraph",
        "chunk_size": 250,  # Smaller chunking for classics
...
def preprocess_lunyu(text: str) -> list[tuple[str, str]]:
    ...
    while i < len(lines):
        ...
        if re.match(r"^[一二三四五六七八九十百]+之[一二三四五六七八九十百]+$", line):
            label = line
            i += 1
            # Find all subsequent lines until the next label or a large gap
            content_lines = []
            while i < len(lines):
...
def main():
    parser = argparse.ArgumentParser(description="Ingest classics into EmbedRAG (Qwen3 Edition)")
    parser.add_argument("--writer-url", default="http://localhost:8002")
    ...
    try:
        ...
        print(
            "Make sure you started the writer with:\n"
            "  uv run python src/embedrag/cli.py writer --config examples/classics_qwen/writer.yaml"
        )
        sys.exit(1)
                    ...
                    documents.append(
                        {
                            ...
                            "text": content,  # Remove label from searchable text
